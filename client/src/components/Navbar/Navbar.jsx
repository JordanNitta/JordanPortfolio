    ...
    return (
        <nav className='fixed w-full flex justify-between h-[68px] items-center sm:px-4 sm:py-2 z-50' >
            ...
            <div onClick={handleMenu} className='block '>
                {!showMenu ? <AiOutlineMenu size={35} className='cursor-pointer sm:hidden md:hidden' /> : <AiOutlineClose size={35} className='cursor-pointer sm:hidden md:hidden' />}
            </div>
            {showMenu && (
                <div className='fixed top-0 left-0 h-screen w-full bg-opacity-90 bg-gray-800 flex justify-center items-center sm:hidden md:hidden -z-30' >
                    <ul className='flex flex-col justify-center items-center' style={{ gap: '0' }} onClick={handleClosedRoutes}>
                        {navLinks.map((link, index) => (
                            <li key={index}>
                                <NavLink to={link.URL} className={({isActive}) => isActive ? 'font-style text-primary-color text-[50px] font-bold' :'font-style text-main text-[60px]'} onClick={handleClosedRoutes}>
                                    {link.title}
                                </NavLink>
                            </li>
                        ))}
